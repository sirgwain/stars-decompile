...
static void save_player0(PLAYER *out_old)
{
    *out_old = rgplr[0];
}

static void restore_player0(const PLAYER *old)
{
    rgplr[0] = *old;
}
...
static void test_FLookupPart_basic_engine_attach(void)
{
    ...
    TEST_CHECK_(part.pengine == &rgengine[iengineQuickJump5],
                "attached engine pointer mismatch: got=%p want=%p",
                (void *)part.pengine, (void *)&rgengine[iengineQuickJump5]);
...
static void test_FLookupPart_disallowed_settlers_delight_requires_HE(void)
{
    ...
    {
        int16_t got = FLookupPart(&part);
        TEST_CHECK_(got == LookupDisallowed,
                    "SettlersDelight without HE: got=%d want=%d", (int)got, (int)LookupDisallowed);
    }
    ...
    {
        int16_t got = FLookupPart(&part);
        TEST_CHECK_(got == LookupOk,
                    "SettlersDelight with HE: got=%d want=%d", (int)got, (int)LookupOk);
    }
