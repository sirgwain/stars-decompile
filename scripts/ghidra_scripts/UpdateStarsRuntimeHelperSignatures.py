...
def run():
    ...
    proto = {
        ...
        "__dos_findfirst":   ("uint16", ["ptr_char", "uint16", "ptr_find_t"]),  # path, attr, find_t*
        "__dos_findnext":    ("uint16", ["ptr_find_t"]),
        "__dos_getdiskfree": ("uint16", ["uint16", "ptr_diskfree_t"]),          # drive, diskfree_t*
    ...
    for (name, addr_s) in entries:
        ...
        if fn is None:
            ...
            try:
                ...
                if fn is None:
                    print("[MISS] %-22s @ %s (no function, could not create)" % (name, addr_s))
                    missing += 1
                    continue
                created += 1
        ...
        try:
            ...
            print("[WARN] %-22s %s (setCallingConvention failed: %s)" % (name, addr_s, e))
        ...
        try:
            ...
            try:
                fn.replaceParameters(FunctionUpdateType.CUSTOM_STORAGE, True, SourceType.USER_DEFINED, params)
            except Exception as e2:
                print("[ERR] %-22s %s (replaceParameters failed: %s)" % (name, addr_s, e2))

        applied += 1
        print("[APPLY] %-22s @ %s  ret=%-10s cc=%s args=%d" %
    ...
    if unsure:
        print("\n[UNSURE] %d functions were given minimal/unknown prototypes:" % len(unsure))
        for nm in unsure:
            print("  - %s" % nm)
